...
- **Admin commands** - `/zigcommerce` for the economy (wallets, storefronts, boards, one player's balances and rerolls) and `/zigprogress` for progression (`quest`, `achievement` and `memories` groups: list the shared catalogue, see where a player stands, give / reset / complete a quest, earn / revoke / reset achievements, forget what conversations remember). Both use engine-derived permission nodes (`ziggfreed.ziggfreedcommon.command.<family>...`), so nobody holds them until you grant them and the console always can. A mod built on CommonLib may register its own aliases over the same engines.

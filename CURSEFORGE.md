...
CommonLib is the plumbing Ziggfreed's mods stand on: quest and achievement engines, an economy of wallets, shops and bounty boards, a branching NPC dialogue engine, placed NPCs, 3D sound, camera effects, toasts and HUD helpers, a co-op instance framework with parties, queues and leaderboards, and a boss framework over the game's own encounter scripts. Shipping it once means every mod built on it behaves the same way and gets fixed in one place. [MMO Skill Tree](https://www.curseforge.com/hytale/mods/mmo-skill-tree), [RPG Stations](https://www.curseforge.com/hytale/mods/rpg-stations), [MMO Mob Scaling](https://www.curseforge.com/hytale/mods/mmo-mob-scaling) and Kweebec Nightmare all run on it.
...
- A boss framework over the game's own encounter scripts: the fight is authored natively, and the library adds credit for the whole party, loot scaled by each player's share, notices, a leaderboard, a map marker and quest progress, from one binding row per fight and with no Java in a pack. Beside it, encounter tools for minigames and dungeons: timed and escalating per-entity effects, hold-this-zone objectives and floor-snapped spawn placement.
...
| Version           | Notes                                                                                                                                                                                                                                                       |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1.1             | Fixes two bugs from 2.1.0. Every bounty board was empty, because publishing a board's contracts and publishing the shared quest catalogue stepped on each other and only the last one to run actually reached the quest system. And a server running a mod built on this library could see its own startup log fill with warnings about quests and achievements clashing with the library's, and lose customizations that should have quietly won. Additive over 2.1.0. |
| 2.1.0             | The boss framework: a fight is one of the game's own encounter scripts, and the library adds credit for the whole party, loot scaled by each player's share, notices, a leaderboard, a map marker and quest and achievement progress, from one binding row per fight, with `/zigencounter` to run and check it. Also: quest steps drawn with a picture and step kinds described by a file, quests that reset on a window of any length, a lifetime tally usable as a condition, the `Flair` reward and `/zigflair`, NPCs answering press F wherever they came from, `/zignpc place` and a placement admin screen, enhanced gear working on a bare server with an authored name and rarity, the placed-block record moving onto the world's own chunks, a shared progress-bar HUD any mod can put a value on (two panels, spots a pack can add, a per-player pick and a settings page, `/zighud`), a notice that repeats rewriting one line of the feed, a payout listing what it handed over, a locked quest saying why and when, and a fix for a server that could freeze for good when a mod wrote a game asset after boot. Requires a Hytale Update 6 (0.6.x) server. MMO Skill Tree 1.6.1, Kweebec Nightmare 1.2.0 and MMO Mob Scaling 1.2.0 require it; update them together. RPG Stations, when it ships, requires it too. |

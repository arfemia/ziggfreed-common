...
- **[`NpcDialogueDepsRegistry`](NpcDialogueDepsRegistry.java) is the decoupling seam, and every registration NAMES ITS OWNER.** A role asset decodes long before a consumer's deps exist, so the action stores only ids and pulls a `Supplier<DialoguePageDeps>` at press-F time. Keyed (normalized trim + lower-case) so a consumer that authors a `DepsKey` reaches its own; the single-consumer case uses `DEFAULT_KEY`. The consumer calls `NpcDialogueDepsRegistry.setDefault(MY_REGISTRY_OWNER, MyDialogue::deps)` once at setup, or the three-argument form for a named key. **Two mods both taking the un-keyed default is a real collision and now says so**: the map is a [`registry/RegistryLedger`](../../../../../../../../zc-core/src/main/java/com/ziggfreed/common/registry/CLAUDE.md), so the later registration takes the slot and prints ONE line naming both owners, instead of swapping silently. The ENGINE is shared and every mod's payload is reachable through [`../dialogue/DialoguePayloads`](../dialogue/CLAUDE.md) whichever deps opened the screen, so what still rides on the deps is narrower than it was - but it is three real things: which resolver looks a conversation up, which header name and hint the page paints, and **which [`i18n/ContentI18n`](../../../../../../../../zc-core/src/main/java/com/ziggfreed/common/i18n/ContentI18n.java) namespace every authored key on the screen resolves through**. The last one has teeth: keys get prefixed with the winning mod's `.lang` filename, so text that resolves on its own server renders as raw keys under somebody else's deps.
...
unit JVM rather than throwing; `NpcDialogueDepsRegistryTest` pins which mod answers a press-F route -
an authored `DepsKey` reaching its own consumer whatever its casing, the un-keyed default being
last-writer-wins, and the ledger reading back which mod holds each key. The rungs that need a running

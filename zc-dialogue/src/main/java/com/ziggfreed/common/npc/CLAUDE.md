...
unit JVM rather than throwing. The rungs that need a running server (an NPC entity's role, native
group membership, a role builder's resolved name key) and the assembly of a credit from a live player
are smoke territory, matching the rest of the mod's split.

...
- **[`NpcTalkDialogue`](NpcTalkDialogue.java)** is the one line joining a conversation's
  ...
  [`DialogueTalk`](../dialogue/DialogueTalk.java), and this package fills that seam. **The dialogue
  ENGINE does not import `npc`** - the direction is `npc -> dialogue`, and a seam is how anything
  goes back. The one exception is [`dialogue/page/DialoguePage`](../dialogue/page/DialoguePage.java),
  which reads `NpcNames` directly for the conversation header: a display name is a read with no
  behaviour behind it, so it needs no seam.
...
**Role JSON shape** (the consumer ships it in its asset pack, `Type: "Generic"`): an `InteractionInstruction` `HasInteracted` block whose `Actions` are `[LockOnInteractionTarget, { "Type": "ZigOpenDialogue", "Dialogue": "<id>" }, { "State": "$Interaction" }]`, plus the `$Interaction`-reset block (load-bearing for repeat press-F). Carry a `NameTranslationKey` or the name renders as the raw role id. Copy kweebec's `KweebecNightmare_Guide.json` shape (`Server/NPC/Roles/KweebecNightmare/`) - a `Type: "Generic"` role whose `HasInteracted` block hardcodes its own conversation id. (The MMO's `Mmo_QuestGiver.json` is NOT the model here: it runs `ZigPlacementInteract` instead, so what press-F opens is written on the placement rather than on the role.)

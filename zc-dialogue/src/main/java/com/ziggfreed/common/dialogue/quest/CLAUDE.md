...
- **[`DialogueQuests`](DialogueQuests.java) is the whole of what this package may reach.** It carries a `QuestStateReader` (zc-progression's narrow READ seam), a `Subject` factory, an answer set, and the two write methods. **`accept` takes the SITE it was taken on** (`accept(subject, questId, siteId)`), and a conversation passes its own context id - the character the player is looking at, never an answered alias - because a quest whose hand-in is "report back to whoever gave me this" can only resolve that from where it started. **`NONE`'s stand-in reader spells out `canCompleteAt` as a refusal rather than inheriting the interface's permissive default**: that default is right for a real runtime (a quest naming no place has no site rule to break), but a stand-in whose every other read says "nothing" would otherwise offer a hand-in for a quest it also says was never started. **Never import `QuestEngine`** - it mutates, and a condition that could reach it could accept a quest while merely deciding whether to render an option. The compile error is the point.
...
- **`subjectOf(store, ref, player)` is the seam for surfaces that are not conversations.** An NPC panel, a fourth party's page and a hand-in button have the player and the world but no dialogue context (the player's reference is read off the entity rather than passed alongside it), and a consumer whose quest runtime needs a richer subject has to be reachable without one. **Override THAT one** when your runtime needs more than a plain player handle; `subject(ctx)` defaults to it and may keep enriching from whatever the conversation already fetched. Overriding only the context form leaves every non-dialogue surface with a bare subject your runtime may not accept.
  - **The two forms answer a missing identity differently, on purpose.** `subjectOf` is reached with real engine handles, so a player whose entity has gone entirely leaves nothing honest to hand back and it refuses loudly. `subject(ctx)` is the RENDER path every quest-aware condition comes through, so it never throws: it falls back to the context's own derived reference and, failing that, builds an UNIDENTIFIED subject (`DialogueQuests.UNIDENTIFIED`, blank name) that every state read answers empty for - the beat draws without quest lines instead of taking the page down.

 ...
 * minigame results-claim, an MMO's bonus mob loot, a luck-loot ground drop) rolls a native drop table
 * through, so the native-roll + in-world-spawn idiom is written once, not re-derived per mod. Mirrors the
 * sibling {@code mmo-mob-scaling} {@code MobScalingLootDropSystem} pattern (native roll via

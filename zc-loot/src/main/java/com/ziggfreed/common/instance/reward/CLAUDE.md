...
`zc-instance`, `zc-presentation`, `zc-commerce` and `zc-encounter` all depend on it - progression and
objectives for the shared reward vocabulary in `loot/reward/`, presentation for what a reward chip
renders, commerce for what a priced offer pays out, encounter for what a defeated boss owes its
participants, instance for this package.

**REVERSE-EDGE TRAP - this module may never import progression, objectives, instance, encounter,
commerce, dialogue, presentation, or world.** Sitting below six consumers means any edge back up is
an immediate cycle. A grant that needs

...
One quest is one file under `Server/ZiggfreedCommon/Quests/`, one achievement one file under `Server/ZiggfreedCommon/Achievements/`, and in both the FILE NAME is the id. Subfolders are free to organise the tree and change nothing, except one whose name starts with `_`, which contributes its name to the id: `_Wilds/First_Camp.json` is `wilds_first_camp`, and marked folders stack. A file may carry `"Parent": "<id>"` to start from another one: every leaf below inherits on omission except `Abstract`, which never carries down, so a child retunes one number and keeps the rest. Any key beginning `$` is authoring metadata the codec ignores, so write a `$Comment` saying what the content does and what each number means in game.

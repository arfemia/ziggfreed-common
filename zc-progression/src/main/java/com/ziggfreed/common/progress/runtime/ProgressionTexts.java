 ...
 * mode this exists to make impossible for any surface, not just the one that noticed it first. It
 * lives beside the runtime it asks, so every surface over that runtime - a book, a board, a
 * character's offer page, a command - reads through the ONE walk instead of hand-rolling its own.
 *
...
public final class ProgressionTexts {
    ...
    /**
     * {@link #title}, or the shared placeholder line when no source knows - for a surface that
    ...
    public static Message titleOrUntitled(@Nonnull String contentId) {
        ...
        return answer != null ? answer : Msg.tr(PREFIX, DOMAIN + "untitled");
    }

    /** {@link #objective}, or the shared placeholder step line when no source knows. */
    @Nonnull
    public static Message objectiveOrUntitled(@Nonnull String contentId, @Nonnull String objectiveId) {
        ...
        return answer != null ? answer : Msg.tr(PREFIX, DOMAIN + "step.untitled");
    }

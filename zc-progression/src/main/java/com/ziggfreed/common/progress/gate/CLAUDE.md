...
| `GateRefusal` (+ `.Kind`) | ONE unmet requirement, structured: the factor id + authored `Param` + BOTH bounds for a factor condition, the quest id, the custom kind id, and the payload-less kinds (permission, `any_of`, `not`). `GateEvaluator.allRefusals` produces them; `allFailures` is their `token()` projection byte for byte, so the records and the tokens can never disagree and the bounds live ONLY on the record - a token is an opaque id, never a channel for numbers. `fromToken` lifts a gate token back into a (bound-less) record for a caller holding only strings; it answers null for anything outside this vocabulary |

...
| `NeutralObjectiveComposer` | the shipped sentence family, convention-keyed on the KIND id (`ziggfreedcommon.progress.objective.<kind>` + `.any` twin + `objective.default`, all nine locales in this module's `ziggfreedcommon.progress.lang`) so a pack-added kind joins by shipping one key - never a closed switch. The authored step key outranks the generated sentence and resolves WITH `{0}`=amount, `{1}`=target name; qualifier and place wrap the result through their own shipped lines; the target's name comes from the engine's shipped catalogues (`i18n.NativeNames.targetNameMsg`), a value-threshold's channel from the factor naming assets (`factor.FactorNames`, the `hytale:stat` overlay) - no vocabulary of its own, and a bare asset id is never painted at a player |

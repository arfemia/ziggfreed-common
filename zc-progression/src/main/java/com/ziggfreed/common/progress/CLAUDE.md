...
| `ContentText` (+ `.Builder`) | what a piece of content is CALLED, carried on the runtime object every fold builds: title / flavor keys with their convention twins and bound args, a per-step key, per-state lore, and a per-step LINE a fold may compose for a step that authored no key. KEYS, not sentences - the one composed leaf is that step line, and it is a `Supplier` because whatever composes it is often installed later in a boot than the catalogue is folded. A NAME has one rung more than a flavor line: when neither key resolves and no name was authored, the first step's own line names the content (what a bounty, or any key-less, name-less quest, reads as on every shared surface; the step lines keep their stamped order for this), and last the explicit key as written, so a row is traceable to its file rather than blank. A convention key a fold derived is never painted as a raw key; an unfilled one on the flavor side answers null, which is what "no line under the title" means. It is what lets ONE text source answer for content authored in any format |
...
- A kind that is SUGAR for another (the file says one thing, the engine should only ever see the
  other) is an `ObjectiveKindRegistry.alias`, registered at setup before the publish, never a second
  fold. The engine never learns an alias exists; a surface never learns the run pair.

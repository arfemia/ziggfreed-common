...
| the quest MOMENTS | `Quest_Objective_Progressed`, `Quest_Completed`, `Quest_Parked` and `Quest_Claimed`, announced from `fireObjectiveProgressed` / `fireCompleted` / `fireClaimed` through the feedback hook. UNCONDITIONAL: `nativeEvents` turns off the cross-mod event bus, never a server's own toasts. Completion is two ids rather than one flag because a quest that settled and one waiting to be collected want their own words and their own sound; `parked` rides both argument maps so a hook can tell the cases apart without reading meaning into the id, and a parked one also carries `reason` (`QuestEngine.PARKED_COLLECT` for a quest authored to be collected, `PARKED_NO_SPACE` when the consumer said the player cannot receive the rewards, `PARKED_AWAY` when the quest names a turn-in site and the player finished elsewhere) plus, for a quest collected somewhere in particular, `turnIn` (`character` / `accept_site`), so ONE authored file can word a full bag apart from a collect-it-later. `Quest_Claimed` carries `collected` (true when the player came back for a parked reward, false when the quest paid out the instant it finished) so a jingle for collecting never plays over the completion jingle. every one of the three carries a payout under `rewards`, and WHICH payout is the before/after rule: `Quest_Parked` fires before anything is paid and carries the PROMISE (`quest.rewards()`, a deferred `Supplier` so a moment nobody authored never composes it), while `Quest_Completed` and `Quest_Claimed` fire AFTER the grant that pays what they announce (`settleAndAnnounce`, shared by the settle-on-the-spot path and `forceComplete`; the native `QuestEvents.fireCompleted` moves with the moment, never split from it) and carry the RECEIPT (`GrantOutcome.receipt()`: what the handlers actually handed over, so a `Lootable` lists the items it rolled, an empty roll adds no row, and a queued or failed reward is absent) - which is what lets an authored toast list what was, or waits to be, handed over. `tryClaim` / `tryForceComplete` are the receipt-answering twins of the boolean `claim` / `forceComplete` (null where those answer false), `trySettle` the twin of the void `checkCompletion` (the settle's outcome, null when nothing was paid now: not complete, or parked), and `tryTurnIn` / `tryAllTurnIns` the twins of the int `attemptTurnIn` / `attemptAllTurnIns` (a `TurnInOutcome`: the same credit plus the payout when the last step settled the quest where it pays out, null when it left the quest open or parked it) - every one for a surface that raises its own toast after the action, and every boolean / void / int form is a thin wrapper over its twin so the two can never drift. Both moments also carry the quest's own `Quest.icon()` under the fixed `icon` name the feedback engine illustrates a notice from, so a quest's notices wear the picture the book shows it as |

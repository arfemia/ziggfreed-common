...
| the quest MOMENTS | `quest.objective_progressed`, `quest.completed`, `quest.parked` and `quest.claimed`, announced from `fireObjectiveProgressed` / `fireCompleted` / `fireClaimed` through the feedback hook. UNCONDITIONAL: `nativeEvents` turns off the cross-mod event bus, never a server's own toasts. Completion is two ids rather than one flag because a quest that settled and one waiting to be collected want their own words and their own sound, and an authored file cannot branch; `parked` rides both argument maps so a hook can tell the cases apart without reading meaning into the id |

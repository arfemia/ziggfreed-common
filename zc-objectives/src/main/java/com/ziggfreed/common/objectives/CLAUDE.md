...
| Package | What it is |
|---|---|
| `runtime/` | `ProgressionBootstrap` (the three setup phases this module owns, each called once from the root's `setup()`: the shared runtime + `/zigprogress`, the authored feedback moments, the default quest-list host) and `ProgressionDefaults`: the default registrations (the library's own `ziggfreedcommon:encounters` axis seeded into `ProgressionRuntime.questAxes()` among them), the asset fold + its audit (`publishAssetContent`, the ONE quest publish: it resolves the store with the runtime's axis vocabulary, folds the layer a consumer contributed into `QuestAssetStore` and the server owner's own `mods/ziggfreedcommon/quests/<Id>.json` folder (re-read by the fold itself), and keeps `questPool()` / `questLoadFindings()` readable for a consumer's audit; it runs FIRST the moment the shared runtime is BUILT, whoever built it - `register()` hangs it on `ProgressionRuntime.onBuilt`, keyed on the PUBLISHED flag, so a ticking system's first engine read publishes exactly as the first player-ready pass does and neither publishes twice; `republishAssetContent` re-runs it on demand once this boot has published, which `/zigprogress reload` and the root registrar's `LoadedAssetsEvent` listeners for quests, generators, achievements and milestones ask for on a hot re-import), the text source, the player lifecycle (the ready pass only makes sure the runtime is built, then runs the player's maintenance), and the `onProgressDirty` / `onProgressFlush` persistence contributions |

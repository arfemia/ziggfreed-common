...
| `validate/` | `EncounterScriptScan` (a pure walk of a script's JSON, following `Reference` and resolving `Compute` reads), `EncounterScripts` (the loaded scripts read back off the builder manager, plus the cached does-it-author-engaged answer the tick asks), `EncounterValidator` (the findings), `EncounterAudit` (the once-gated boot pass at first player ready and `/zigencounter validate`), `EncounterSelfTest` (the base and the example must resolve by index; SEVERE otherwise) |

...
| `payout/` | `EncounterLoot` (`Loot.OnDefeat` per credited participant through `LootEngine.select` then `RewardGrants.grantAll`, each roll kept with probability = share, offline participants queued; `grantDefeat` / `grantShares` answer each paid player's own RECEIPT, the grant pass's `GrantOutcome.receipt()` keyed by player id, empty-roll and queued payouts absent; `Loot.OnPhase` spilled in world), `EncounterFeedback` (a moment per member behind the subject's own `FeedbackAudience`, `share` and `rank` as typed numbers, and at a defeat each participant's OWN receipt under `rewards` through `fireWithShares(..., receipts)` / `ownArgs`, so the in-page toast lists what that player's roll landed and never another player's), `EncounterDiscovery` (the world-map marker, placed, followed, removed) |

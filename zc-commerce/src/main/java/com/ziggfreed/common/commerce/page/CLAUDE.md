...
| [`CommerceChips`](CommerceChips.java) | the two repeatable things both pages paint: a chip and a line; also the amount-and-name compositions - `priceAmount`/`amountAndName` ("50 Bounty Tokens", no sign) and `rewardAmount`/`rewardAmountAndName` ("+50 Bounty Tokens", the reward-row plus), two SEPARATE `ziggfreedcommon.commerce` keys because a price and a reward never share wording. Every display number here binds as a TYPED numeric param (`Msg.num` / the composition keys) so the client groups digits per locale - never `Msg.raw(NumberFormatter...)`; the root `NumberDisplayHygieneTest` fails the build on it |
| [`CurrencyText`](CurrencyText.java) | what a wallet is CALLED, on a three-rung ladder |
| [`CurrencyChipReading`](CurrencyChipReading.java) | how a `Currency` reward READS anywhere in the library, not only here: the wallet's own icon and the reward-row amount-and-name on the same three-rung ladder. Contributed once into the shared `RewardChips` by the wiring root, so a reward that pays a wallet never has to author a `NameKey` and one that does still wins; a wallet no layer defines answers null and the chip stays dropped |
| [`CommerceStepIcons`](CommerceStepIcons.java) | what a step about this module's own vocabulary is DRAWN with: a wallet by its own icon, a board by the picture it is listed under. Contributed once into the shared `ProgressionIcons` by the wiring root, because neither id is an item id and the engines that list steps have no edge to this module; the kind carries the FACT (`TargetNames.Currency` / `TargetNames.Board` on its file), this carries the READING, and an id no layer defines answers null so the step falls back to its kind's own picture |

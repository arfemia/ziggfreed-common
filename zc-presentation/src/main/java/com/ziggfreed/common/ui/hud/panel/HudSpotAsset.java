 ...
 * FILE NAME is the spot's id, what a panel's {@code Placement} names, what a player picks in
 * the HUD settings and what {@code /zighud place} takes.
 *
 * <p>Authored at {@code Server/ZiggfreedCommon/HudSpots/<id>.json}. This library ships
 * {@code Top_Left.json}, {@code Top_Right.json} and {@code Bottom_Left.json}; a pack adds a spot by
 * dropping one more file here and every picker offers it, and a server owner retunes any of them
 * from {@code mods/ziggfreedcommon/hud-spots.json} by id.
 * <pre>{@code
 * // Server/ZiggfreedCommon/HudSpots/AboveHotbar.json
 * { "LabelKey":      "mypack.hud.spot.above_hotbar",
 ...
 * ({@link HudSpotCutout}) leaves the cells at the pinned end of ONE column empty, so the panel steps
 ...
 */
public final class HudSpotAsset
        implements JsonAssetWithMap<String, DefaultAssetMap<String, HudSpotAsset>> {

    /** Where these are authored. */
    public static final String TYPE_ROOT = "ZiggfreedCommon/HudSpots";

    /** The Asset Editor pick list a panel's {@code Placement} leaf is chosen from: every spot id. */
    public static final String EDITOR_DATA_SET = "ziggfreedcommon:hud_spots";

    /** Where a spot sits in a picker when it names no order: after every one that did. */
    ...
    public static final AssetBuilderCodec<String, HudSpotAsset> CODEC = AssetBuilderCodec.builder(
                    HudSpotAsset.class,
                    HudSpotAsset::new,
                    Codec.STRING,
                    (a, id) -> a.id = id,
            ...
            .documentation("A localization key naming this spot in the HUD settings, resolved on each "
                    + "player's own client. A full registered id (\"mypack.hud.spot.corner\") passes "
                    + "through as written. Left out, the spot is listed by its id.")
            .add()
    ...
    /** The label's localization key, trimmed, or null to list the spot by its id. */
    @Nullable
    public String labelKey() {
        return labelKey == null || labelKey.isBlank() ? null : labelKey.trim();
    }

    /** What the spot is called on screen: its key resolved on the client, else its id. */
    @Nonnull
    ...
    @Nullable
    public HudSpotPosition position() {

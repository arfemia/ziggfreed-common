 ...
 * reports it moved ({@link HudPanels#moved}, {@link HudPanels#itemMoved}), and that mod supplies the
 * row's whole look along with the movement ({@link HudRowDisplay}). A file here is for the server
 ...
 *
 * <p>Authored at {@code Server/ZiggfreedCommon/HudRows/<id>.json} (this codec IS the schema):
 * <pre>{@code
 * // Server/ZiggfreedCommon/HudRows/Wood.json
 * { "Source":   "WOOD",
 *   "Color":    "#6fbf73",
 *   "Order":    20,
 *   "LingerMs": 8000 }
 ...
 * {@code "Parent": "<override id>"} key, and a server owner's {@code mods/ziggfreedcommon/hud-rows.json}
 * entry restates only the leaves it wants different over whatever a pack authored under that id.
 */
public final class HudRowAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, HudRowAsset>> {

    /** Where these are authored. One folder, one file per override, the file name being the id. */
    public static final String TYPE_ROOT = "ZiggfreedCommon/HudRows";
    ...
    public static final AssetBuilderCodec<String, HudRowAsset> CODEC = AssetBuilderCodec.builder(
                    HudRowAsset.class,
                    HudRowAsset::new,
                    Codec.STRING,
                    (a, id) -> a.id = id,
            ...
            .metadata(EditorSchema.defaultValue(HudRowLook.DEFAULT_COLOR))
            .documentation("The fill colour as a six-digit hex, e.g. \"#6fbf73\", in place of the colour the "
                    ...
                    + "stands, and a mod that named none reads " + HudRowLook.DEFAULT_COLOR + ".")
            .add()
            .appendInherited(new KeyedCodec<>("Order", Codec.INTEGER, false),
                    (a, v) -> a.order = v, a -> a.order, (a, p) -> a.order = p.order)
            .metadata(EditorSchema.defaultValue(HudRowLook.DEFAULT_ORDER))
            .documentation("Where this row sits when several are up at once: lower numbers draw higher in "
                    ...
                    + "named none reads " + HudRowLook.DEFAULT_ORDER + ", after every row that did.")
            .add()
            .appendInherited(new KeyedCodec<>("LingerMs", Codec.LONG, false),
                    (a, v) -> a.lingerMs = v, a -> a.lingerMs, (a, p) -> a.lingerMs = p.lingerMs)
            .metadata(EditorSchema.defaultValue(HudRowLook.DEFAULT_LINGER_MS))
            .documentation("How long the row stays on screen after its value last moved, in milliseconds; "
                    ...
                    + "and a mod that named none reads " + HudRowLook.DEFAULT_LINGER_MS + ".")
            .add()
     ...
     * ({@link HudRowDisplay#over}): the label key becomes a client-resolved message, and every
     * unauthored leaf stays null so it changes nothing.
     */
    @Nonnull
    public HudRowDisplay display() {
        String key = labelKey();
        return new HudRowDisplay(key != null ? ContentKeys.tr(key) : null, icon(), color(), order(), lingerMs(),
                null, null, null);
    }
}

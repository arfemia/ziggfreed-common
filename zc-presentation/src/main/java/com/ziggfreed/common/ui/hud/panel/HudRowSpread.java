 ...
 * every column's shape ({@link HudPanelHud.ColumnShape}) and every slot's row are read from. Rows are
 ...
 */
record HudRowSpread(@Nonnull List<int[]> columns) {

    /** Nothing showing: no column and no depth. */
    static final HudRowSpread EMPTY = new HudRowSpread(List.of());
     ...
     */
    @Nonnull
    static HudRowSpread of(int rowCount, int perColumn, int slotsPerColumn, @Nullable HudSpotCutout cutout) {
        if (rowCount <= 0 || perColumn <= 0 || slotsPerColumn <= 0) {
            return EMPTY;
        }
        // The split: perColumn to each column in turn, the last one short.
        int used = HudPanelHud.usedColumnsFor(rowCount, perColumn);
        ...
        return new HudRowSpread(List.copyOf(columns));
    }
     ...
     * ({@link HudPanelHud#ordinalInColumn}), so a column shorter than it hides its surplus slots.
     */
    int depth() {
        int depth = 0;

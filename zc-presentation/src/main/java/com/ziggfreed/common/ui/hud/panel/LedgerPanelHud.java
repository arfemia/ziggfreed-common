 ...
 *
 * <p>Where it sits is its {@code Activity_Ledger.json} panel file's {@code Placement}, the
 * shipped {@code Top_Left} spot: the same origin as the zone card a companion mod draws at the
 * top-left, deliberately OVER it, since this panel attaches after that card at ready and is
 * transparent behind its frame. That spot states a {@code MinHeight} of the card's own height, so
 * a ledger of one or two rows still covers the card rather than leaving its lower half showing;
 * the floor belongs to the spot, applies to whichever panel sits there, and is tuned in its file.
 * The corner below is only the document's fallback for a server where that file is gone.
 */
public final class LedgerPanelHud extends HudPanelHud {

    /** This panel's key on the native per-player {@code HudManager}, under this library's own id. */
    public static final String HUD_KEY = "ziggfreedcommon:activity_ledger";
    ...
    public LedgerPanelHud(@Nonnull PlayerRef playerRef) {

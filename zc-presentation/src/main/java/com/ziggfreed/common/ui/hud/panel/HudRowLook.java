 ...
 * {@link HudRowAsset} authored for the row (if any) over the {@link HudRowDisplay} its movement
 ...
 */
public record HudRowLook(@Nonnull Message label, @Nullable IconSpec icon, @Nonnull String color, int order,
     ...
     * sends it away ({@link HudPanels#fadeAll}). For a row that belongs to a stretch of activity with
     ...
     */
    @Nonnull
    public static HudRowLook resolve(@Nonnull String id, @Nullable HudRowAsset override,
            @Nonnull HudRowDisplay display) {
        HudRowDisplay folded = override == null ? display : override.display().over(display);
        Long linger = folded.lingerMs();
        return new HudRowLook(
